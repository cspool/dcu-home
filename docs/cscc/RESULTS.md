...
补丁相对直接代码基线为新增 896 行、删除 94 行，保守改动量共 990 行；压缩前
为 2656 行，因此减少 1666 行（62.7%）。
...
kernel，不使用投机解码。当前压缩版三档首轮聚合相对压缩前为 `-0.580%`，
匹配重复后的聚合差异为 `+0.195%`。双 rank 冷编译、稳态重启条件、并发
2/4/8 数据以及同负载 DP1 对照见 [DP2_MULTI_REQUEST.md](DP2_MULTI_REQUEST.md)。多请求
