    ...
    def check_and_update_config(cls, vllm_config: "VllmConfig") -> None:
        ...
        if cache_config and not cache_config.user_specified_block_size:
            if (
                ...
                # Hybrid models (for example Qwen3.5) may already have raised
                # the attention block size so it can share pages with their
                # recurrent layers.  Replacing that model-selected value with
                # AITER's generic 64-token default makes the two page sizes
                # impossible to unify (784 is not divisible by 64).
                if cache_config.block_size <= 64:
                    cache_config.block_size = 64
                    logger.warning(
                        "[ROCM_AITER_UNIFIED_ATTN]: Setting kv cache block size "
                        "to 64."
                    )
                else:
                    logger.info(
                        "[ROCM_AITER_UNIFIED_ATTN]: Keeping model-selected kv "
                        "cache block size %d.",
