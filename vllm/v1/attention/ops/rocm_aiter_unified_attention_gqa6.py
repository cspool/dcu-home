...
# Role: handle Qwen3.5 TP=2 Q12/KV2/D256 (and TP=1 Q24/KV4/D256) prefill
# when page784 declines the request.
...
def _gqa6_prefill(
    ...
    head_pairs_per_kv: tl.constexpr = Q_HEADS_PER_KV // 2
    kv_head = query_group // head_pairs_per_kv
    head_pair = query_group % head_pairs_per_kv
    ...
    query_head = kv_head * Q_HEADS_PER_KV + head_pair * 2 + rows % 2
    query_offset = local_token[:, None] * QUERY_STRIDES[0]
    query_offset += query_head[:, None] * QUERY_STRIDES[1]
    query_offset += dimensions[None, :] * QUERY_STRIDES[2]
    ...
    output_offset = local_token[:, None] * OUTPUT_STRIDES[0]
    output_offset += query_head[:, None] * OUTPUT_STRIDES[1]
    output_offset += dimensions[None, :] * OUTPUT_STRIDES[2]
...
def prefill(
    ...
    assert heads_per_kv == 6 and num_query_heads % 2 == 0
