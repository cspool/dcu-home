...
_H11_3_SHORT_PREFILL_COMPILER_CONFIG = {
    "num_warps": 4,
    "num_stages": 2,
    "waves_per_eu": 1,
}
_H11_4_LONG_PREFILL_COMPILER_CONFIG = {
    "num_warps": 2,
    "num_stages": 1,
    "waves_per_eu": 1,
...
_H11_5_WIDE_CAUSAL_PREFILL_COMPILER_CONFIG = {
    "num_warps": 4,
    "num_stages": 1,
    "waves_per_eu": 1,
    "matrix_instr_nonkdim": 16,
...
def kernel_unified_attention_2d_gqa6(
    ...
    tl.static_assert(
        num_queries_per_kv == HEADS_PER_CTA * GQA_SPLITS,
        "GQA heads must be partitioned exactly",
    )
    tl.static_assert(
        BLOCK_M == BLOCK_Q * HEADS_PER_CTA,
        "BLOCK_M must contain only complete token/head groups",
    )
    ...
    # H11.3: 16 rows = 8 query positions * 2 heads.  In contrast, dividing
    # by GQA6 directly maps the last four rows into the next q-block.
    ...
    query_offset_1 = (
        ...
        + q_head_group_idx * HEADS_PER_CTA
        + offs_m % HEADS_PER_CTA
    )
    query_offset = (
    ...
    # H11.5: this is causal prefill, so a query CTA never needs K/V blocks
    # beyond its final query position.  The original AITER loop walked to the
    # end of the whole query chunk for every CTA and only discarded future
    # positions with seq_mask below.  Tightening the dynamic loop bound avoids
    # those future K/V loads and dot products while preserving the per-row
    # causal mask inside the last block.
    causal_query_stop = tl.minimum(
...
def unified_attention_gqa6_prefill(
    q,
    k,
    v,
    out,
    ...
    del max_seqlen_k  # Kept in the signature for drop-in AITER compatibility.
    ...
    # Keep AITER's original block-size policy exactly.  The target workload has
    # CACHE_BLOCK_SIZE=784 and selects BLOCK_SIZE=16.
    if block_size * head_size_padded * element_size > 16384:
        ...
        assert block_size is not None, (
            "cannot find a suitable block size for H11.3 unified attention"
        )
    ...
    # H11.4: the long-prefill kernel is register/LDS constrained on gfx936.
    # matrix_instr_nonkdim=16 + kpack=2 lets Triton use the native MFMA
    # layout, while two warps and a single stage reduce the compiled resource
    # footprint from 253 VGPR / 16896 B LDS to 175 VGPR / 8192 B LDS without
    # changing the launch grid or attention math.  Very small tail chunks have
    ...
    # Preserve AITER's CPU-free upper bound for variable-length batches.
    total_num_q_blocks = q.shape[0] // block_q + num_seqs

    kernel_unified_attention_2d_gqa6[
        (total_num_q_blocks, num_kv_heads, gqa_splits)
    ](
        output_ptr=out,
        query_ptr=q,
        key_cache_ptr=k,
