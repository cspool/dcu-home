...
def _pack_page784(
    ...
    target = token * PACKED_STRIDES[0] + head * PACKED_STRIDES[1]
    target += dimensions * PACKED_STRIDES[2]
...
def _merge_page784(
    ...
    token = row // NUM_QUERY_HEADS
    head = row % NUM_QUERY_HEADS
...
def _page_workspace(
    query: torch.Tensor, page_count: int, num_kv_heads: int
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    num_query_heads = query.shape[1]
    key = (query.device, query.dtype, num_query_heads, num_kv_heads)
    if key not in _PAGE_WORKSPACE:
        ...
        token_buffers = (
            torch.empty((4096, num_query_heads, 256), **options) for _ in range(2)
        )
        page_buffers = (
            torch.empty((160, 64, num_kv_heads, 256), **options) for _ in range(2)
        )
...
def prefill(
    ...
    supported &= num_query_heads == num_kv_heads * 6
    if not supported:
        return False
    ...
    main, residual, packed_key, packed_value = _page_workspace(
        query, packed_pages, num_kv_heads
    )
    packed_key_tokens = packed_key.view(-1, num_kv_heads, 256)
    packed_value_tokens = packed_value.view(-1, num_kv_heads, 256)
    block_table = metadata.block_table
    query_starts = metadata.query_start_loc

    _pack_page784[(combined_tokens, num_kv_heads)](
        key_cache,
        value_cache,
        current_key,
        current_value,
        ...
        PACKED_STRIDES=packed_key_tokens.stride(),
        num_warps=4,
    )
    ...
    _merge_page784[(triton.cdiv(query_len * num_query_heads, 4),)](
        output,
        main,
        main_lse,
        residual,
